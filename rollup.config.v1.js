...
const testConfig = {
  input: "packages/components/loading/index.less",
  output: {
    file: "lib/loading.css",
  },
  plugins: [
    postcss({
      extract: true,
...
function createStyleConfig(src) {
  const styleConfigs = _.reduce(
    ...
    (configs, dir) => {
      configs.push({
        ...
        plugins: [
          postcss({
            extract: path.resolve(`lib/style/${dir}.css`),
            extensions: [".less"],
          }),
        ],
      });
    ...
    [
      {
        ...
        plugins: [
          postcss({
            extract: path.resolve(`lib/style/index.css`),
            extensions: [".less"],
          }),
        ],
      },
  ...
  console.log(styleConfigs);
  return styleConfigs;
}
const configs = createStyleConfig(getPath("packages/components"));
